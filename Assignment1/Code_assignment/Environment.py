...
class StochasticWindyGridworld:
    ...
    def _initialize_plot(self):
        ...
        self.ax.add_patch(Rectangle(self.start_location,1,0,angle=0, linewidth=0, facecolor='b',alpha=0.2))
        ...
        for i in range(len(self.goal_locations)): 
            if self.goal_rewards[i] >= 0:
                ...
                colour = 'r'
                text = '{}'.format(self.goal_rewards[i])
            self.ax.add_patch(Rectangle(self.goal_locations[i],1,0,angle=0, linewidth=0, facecolor=colour,alpha=0.2))
